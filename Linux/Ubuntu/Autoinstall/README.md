...
**Description**: Add here any extra packages required for your . Make sure to add the correct sources under the APT section if the package is not on the default Ubuntu's repository (example: microsoft-edge-stable and intune-portal)<br>
...
**Description**: This section is optional<br>
...
```
Check's if the APT Repository List file `microsoft-edge.list` file exists and if it does removes it. During the testing of this autoinstall file, the unattended installation failed due to APT returning a code 2. Uppon reading the logs, this error was pointing to a duplicated sources list entry; one of the files was generated by the entry on the autoinstall yaml file on the apt section (`microsoft-edge-stable.list`) and another one was automatically generated (`microsoft-edge.list`). Not sure what is generating this file but is causing autoinstall to not complete sucessefully. This next line is **optional**.
```yaml
